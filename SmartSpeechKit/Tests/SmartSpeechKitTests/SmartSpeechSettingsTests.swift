...
@Suite("SmartSpeechSettings")
struct SmartSpeechSettingsTests {
    @Test("Defaults match the spec")
    func defaults() {
        let s = SmartSpeechSettings()
    ...
    func presetProgression() {
        let presets = [SmartSpeechSettings(preset: .default),
                       SmartSpeechSettings(preset: .more),
                       SmartSpeechSettings(preset: .aggressive)]
    ...
    func defaultPresetMatches() {
        #expect(SmartSpeechSettings(preset: .default) == SmartSpeechSettings())
    }

    @Test("Codable round-trip")
    func codable() throws {
        let original = SmartSpeechSettings(minSilenceDuration: 0.3, minKeptSilence: 0.2,
                                       residualSlope: 0.15, thresholdMarginDb: 9,
                                       edgeGuardMs: 35, crossfadeMs: 18)
        let data = try JSONEncoder().encode(original)
        let decoded = try JSONDecoder().decode(SmartSpeechSettings.self, from: data)
