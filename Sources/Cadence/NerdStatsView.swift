...
/// collapsed while trimming was active. Lifetime totals live in `CadenceStats` (UserDefaults), polled
...
struct NerdStatsView: View {
    ...
    private var reclaimedBooks: [Audiobook] {
        books
            .filter { ($0.cadenceSavedSeconds ?? 0) > 0 }
            .sorted { ($0.cadenceSavedSeconds ?? 0) > ($1.cadenceSavedSeconds ?? 0) }
    }
    private var maxSaved: Double { reclaimedBooks.first?.cadenceSavedSeconds ?? 1 }
        ...
        NavigationStack {
            ...
            .task {
                while !Task.isCancelled {
                    totalPlayed = CadenceStats.totalPlayedSeconds
                    totalSaved = CadenceStats.totalSavedSeconds
