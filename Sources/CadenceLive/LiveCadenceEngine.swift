...
/// EXPLORATION MODULE — the live Cadence playback engine (the AVPlayer alternative the shipped
...
///
/// Everything the UI reads is SOURCE-domain (matching the shipped Cadence invariant): `sourcePosition`
...
final class LiveCadenceEngine {
    ...
    private(set) var preset: CadenceSettings.Preset = .default
    ...
    private var source: LiveCadenceSource?
    ...
    func load(book: Audiobook) {
        guard let src = LiveCadenceSource(book: book) else { teardown(); status = "No playable file"; return }
        configure(with: src)
    }
    ...
    func load(debugFileURL url: URL, preset: CadenceSettings.Preset = .default) {
        guard let src = LiveCadenceSource(debugFileURL: url, preset: preset) else {
    ...
    private func configure(with src: LiveCadenceSource) {
        teardown()
        self.source = src
        ...
        let producer = LiveTrimProducer(url: src.url, cutPoints: src.cutPoints,
                                        ...
                                        settings: LiveCadenceTuning.settings(preset: src.preset),
    ...
    private func runPrescan(src: LiveCadenceSource) {
    ...
    func applyTrim(enabled: Bool, preset: CadenceSettings.Preset) {
        self.trimEnabled = enabled
        self.preset = preset
        producer?.configure(settings: LiveCadenceTuning.settings(preset: preset), trimEnabled: enabled)
