...
/// EXPLORATION MODULE — DEBUG-only screen to drive the live (on-the-fly) Cadence engine against a
...
struct LiveCadencePlayerView: View {
    @Query(sort: \Audiobook.title) private var books: [Audiobook]
    @State private var engine = LiveCadenceEngine()
    ...
    var body: some View {
        ...
        .navigationTitle("Live Cadence (spike)")
    ...
    private var trimSection: some View {
        Section("Trim") {
            ...
            Picker("Tier", selection: Binding(
                ...
                ForEach(CadenceSettings.Preset.allCases, id: \.self) { p in
    ...
    private var diagnosticsSection: some View {
        Section("Diagnostics") {
            ...
            LabeledContent("Silence ceiling (live)", value: String(format: "%.0f dB", LiveCadenceTuning.silenceCeilingDb))
            Button {
                exportTrimmed()
            } label: {
    ...
    private func exportTrimmed() {
        guard let book = selectedBook, let src = LiveCadenceSource(book: book) else { return }
        exporting = true
        exportStatus = "Exporting…"
        Task.detached(priority: .userInitiated) {
            do {
                let outURL = try LiveCadenceExport.exportTrimmed(url: src.url, preset: src.preset)
                await MainActor.run {
                    exporting = false
