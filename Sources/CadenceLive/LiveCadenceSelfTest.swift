...
/// EXPLORATION MODULE — headless smoke test for the live Cadence engine, triggered by the launch
/// argument `-livecadenceselftest`. Drives the engine against the bundled `Sample Chaptered.m4b`
...
@MainActor
enum LiveCadenceSelfTest {
    static var isRequested: Bool { CommandLine.arguments.contains("-livecadenceselftest") }

    private static func log(_ s: String) { print("LIVECADENCE-SELFTEST: \(s)") }

    static func run() async {
        guard let url = bundledFixtureURL() else { log("FAIL: no bundled fixture found"); return }
        log("fixture: \(url.lastPathComponent)")

        let engine = LiveCadenceEngine()
    ...
    private static func runMusicNarrationDiagnostic() {
        ...
        let ceiling = CadenceSettings().absoluteSilenceCeilingDb
        ...
        if let r = try? LiveSilencePrescan.analyze(url: music, cutPoints: [0], preset: .aggressive) {
            ...
            let settings = CadenceSettings(preset: .aggressive)
    ...
    private static func runCeilingDiagnostic() {
        ...
        let s = CadenceSettings(preset: .aggressive)
        log("CEIL room-tone floor=\(fmt(profile.noiseFloorDb))dB speech=\(fmt(profile.speechLevelDb))dB")
        for ceil in [-50.0, -40.0, -30.0, LiveCadenceTuning.silenceCeilingDb] {
        ...
        if let r = try? LiveSilencePrescan.analyze(url: room, cutPoints: [0], preset: .aggressive) {
            log(r.projectedSavedSeconds > 1
                ? "CEIL PASS: live pre-scan trims normal recording (\(fmt(r.projectedSavedSeconds))s, floor=\(fmt(r.globalFloorDb))dB, liveCeiling=\(fmt(LiveCadenceTuning.silenceCeilingDb))dB)"
