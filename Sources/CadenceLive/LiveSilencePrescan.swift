...
/// Live-only detection tuning. Kept separate from the shipped `CadenceSettings` defaults so the
/// spike can be tuned without touching the pre-render feature.
enum LiveCadenceTuning {
    ...
    /// CadenceSettings for the live path: tier preset with the relaxed live ceiling applied.
    static func settings(preset: CadenceSettings.Preset) -> CadenceSettings {
        var s = CadenceSettings(preset: preset)
        s.absoluteSilenceCeilingDb = silenceCeilingDb
        return s
    }
}

/// EXPLORATION MODULE — live (on-the-fly) Cadence. See `specs/realtime-cadence-exploration.md`
/// and the plan `~/.claude/plans/the-app-currently-uses-streamed-corbato.md`. Isolated from the
/// shipped pre-render Cadence pipeline; nothing here touches `AudiobookPlayer`/`CadenceRenderer`.
///
...
///
/// Reuses CadenceKit verbatim (`AudioIO`, `SilenceAnalyzer`, `SilencePolicy`) and the app's existing
/// `CadenceRenderer.chunkWindows` so the chunking matches the pre-render oracle exactly — which is
/// what makes the M0 assertion (prescan projection == `CadenceRenderer` projection) meaningful.
struct LiveSilencePrescanResult: Sendable {
    ...
    /// `CadenceRenderer.projectedSaved` / `TrimReport`'s ideal figure (actual rendered saving is a
...
enum LiveSilencePrescan {
    ...
    static func analyze(url: URL,
                        ...
                        preset: CadenceSettings.Preset) throws -> LiveSilencePrescanResult {
        ...
        for w in windows {
            try autoreleasepool {
                ...
                let mono = AudioIO.downmixToMono(buffer)
                let profile = SilenceAnalyzer.profile(monoSamples: mono, sampleRate: sampleRate)
                for tierPreset in CadenceSettings.Preset.allCases {
                    let tierSettings = LiveCadenceTuning.settings(preset: tierPreset)
    ...
    /// `CadenceRenderer.projectedSaved` (kept private there) — reproduced so the spike stays isolated.
