...
/// Time Saved stat card for SmartSpeech (silence-trimming) feature.
///
/// A rounded card displaying the cumulative time saved across all audiobooks
/// with SmartSpeech enabled. Shows an empty state when no time has been saved yet,
...
/// - Empty state: helper text "Start listening with SmartSpeech on to see your saved time."
/// - Populated state: large bold duration (e.g. "4h 12m") + subtitle with book count.
/// A reusable SmartSpeech stat card (icon + uppercase title + big value + optional subtitle, on a
...
struct SmartSpeechStatCard: View {
    let icon: String
    let title: String
...
struct SmartSpeechTimeSavedCard: View {
    ...
    var body: some View {
        SmartSpeechStatCard(
            ...
            emptyText: "Start listening with \(SmartSpeechBranding.featureName) on to see your saved time.",
...
#Preview("Populated") {
    VStack(spacing: 20) {
        SmartSpeechTimeSavedCard(totalSeconds: 15120, bookCount: 3)
        SmartSpeechTimeSavedCard(totalSeconds: 780, bookCount: 1)
        SmartSpeechTimeSavedCard(totalSeconds: 45, bookCount: 2)
    }
    .padding()
}

#Preview("Empty") {
    SmartSpeechTimeSavedCard(totalSeconds: 0, bookCount: 0)
        .padding()
}
